...
def test_graceful_degradation_when_missing():
    ...
    # In the real code this depends on the import block. We can mock the flag.
    with patch("envfix.embeddings._HAS_SENTENCE_TRANSFORMERS", False):
        assert envfix.embeddings.get_embedding("test") is None
        assert envfix.embeddings.cosine_similarity([1.0], [1.0]) == 0.0


@patch("envfix.embeddings.SentenceTransformer")
@patch("envfix.embeddings.util")
def test_successful_embedding(mock_util, mock_st_class):
    with patch("envfix.embeddings._HAS_SENTENCE_TRANSFORMERS", True):
